        ...
        <div class="shop-sidebar side-sticky bg-body" id="shopFilter">
                    ...
                    <div class="accordion-body px-0 pb-0 pt-3">
                        <ul class="list list-inline mb-0">
                            @foreach ($categories as $category)
                                <li class="list-item">
                                    <form class="menu-link py-1" action="{{route('shop.index')}}" id="categories-form" method="get">
                                        <input @if(in_array($category->id, explode(",", $f_categories))) checked @endif data-id="{{$category->id}}" onchange="handlerFormSubmit(event)" type="checkbox" class="chk-category">
                                        {{$category->name}}
                                    </form>
                        ...
                        <div class="search-field multi-select accordion-body px-0 pb-0">
                            ...
                            <ul class="list list-inline mb-0 brand-list">
                                @foreach ($brands as $brand)
                                    <li class="list-item">
                                        <form action="{{route('shop.index')}}" method="GET" id="brands-form" class="menu-link py-1">
                                            <input @if (in_array($brand->id, explode(",", $f_brands))) checked="checked" @endif data-id="{{$brand->id}}" onchange="handlerFormSubmit(event)" type="checkbox" class="chk-brand" />
                                            {{$brand->name}}
                                        </form>
            ...
            <div class="accordion" id="price-filters">
                ...
                <div id="accordion-filter-price" class="accordion-collapse collapse show border-0"
                    ...
                    <input onchange="handlerFormSubmit(event)" class="price-range-slider" type="text" name="price_range" value="" data-slider-min="0" data-slider-max="500" data-slider-step="5" data-slider-value="[{{$min_price}},{{$max_price}}]" data-currency="$" />
            ...
            <div class="d-flex justify-content-between mb-4 pb-md-2">
                ...
                <div class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1">
                    ...
                    <form action="{{route('shop.index')}}" method="get">
                        <select onchange="handlerFormSubmit(event)" class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" id="size" aria-label="Page Size" name="size">
                            <option value {{$size === "9" ? "selected" : ""}}>Show Items</option>
                    ...
                    {{-- order --}}
                    <form action="{{route('shop.index')}}" method="get">
                        <select onchange="handlerFormSubmit(event)" class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" aria-label="Sort Items" name="order">
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="{{route('shop.product', ["slug"=>$product->slug])}}"><img loading="lazy" src="{{asset("uploads/products")}}/{{$product->image}}" width="330"
                                                height="400" alt="{{$product->name}}" class="pc__img"></a>
                                        </div>
                                        @if ($product->images)
                                            @foreach (explode(", ",$product->images) as $img)
                                                <div class="swiper-slide">
                                                    <a href="{{route("shop.product", ["slug"=>$product->slug])}}"><img loading="lazy" src="{{asset('uploads/products')}}/{{$img}}" width="330"
                                                        height="400" alt="{{$product->name}}" class="pc__img"></a>
                                                </div>
                                            @endforeach
                                        @endif
                            ...
                            <div class="pc__info position-relative">
                                ...
                                <h6 class="pc__title"><a href="{{route('shop.product', ["slug"=>$product->slug])}}">{{$product->name}}</a></h6>
                                <div class="product-card__price d-flex">
                                    @if($product->sale_price > 0)
                                        <span class="money price"><s>{{$product->regular_price}}</s> {{$product->sale_price}}</span>
                                    @else
                                ...
                                @if (Cart::instance("wishlist")->content()->where("id", $product->id)->count() > 0)
                                    <form action="{{route('wishlist.delete.item', ['rowId' => Cart::instance("wishlist")->content()->where("id", $product->id)->first()->rowId])}}" method="POST">
                                        @csrf
                                        @method("DELETE")
                                        <button onclick="event.target.closest('form').submit()" class="pc__btn-wl position-absolute top-0 end-0 bg-transparent border-0 js-add-wishlist" title="Add To Wishlist">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 24 24"><path fill="#9c7539" d="M12.001 4.529a6 6 0 0 1 8.242.228a6 6 0 0 1 .236 8.236l-8.48 8.492l-8.478-8.492a6 6 0 0 1 8.48-8.464"/></svg>
                                        </button>
                                    </form>
                                @else
...
@push("scripts")
    <script>
        function handlerFormSubmit (e) {
            ...
            const minPrice = form.querySelector("input[name=min]")
            const maxPrice = form.querySelector("input[name=max]")

            const valSize = document.querySelector("select[name=size]")
            const valOrder = document.querySelector("select[name=order]")
            const valPrice = document.querySelector("input[name=price_range]").value.split(",")
            

            size.value = valSize.value
            order.value = valOrder.value
            minPrice.value = valPrice[0]
            maxPrice.value = valPrice[1]
        ...
        function updateBrands () {
            const checkedInputs = document.querySelectorAll('#brands-form input[type=checkbox]:checked')
            const brands = document.querySelectorAll("input[name=brands]")
            const vals = []

            if (checkedInputs.length > 0 && brands.length > 0) {

                checkedInputs.forEach(el => {
                    vals.push(el.getAttribute("data-id"))
                })
        ...
        function updateCategories () {
            const checkedInputs = document.querySelectorAll("#categories-form input[type=checkbox]:checked")
            const categories = document.querySelectorAll("input[name=categories]")
            const vals = []

            if (checkedInputs.length > 0 && categories.length > 0) {
                checkedInputs.forEach (el => {
                    vals.push(el.getAttribute("data-id"))
                })
