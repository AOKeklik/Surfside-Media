            ...
            <div class="product-single__media" data-media-type="vertical-thumbnail">
                <div class="product-single__image">
                    <div class="swiper-container">
                        ...
                        @foreach (explode(", ", $product->images) as $productImg)
                            <div class="swiper-slide product-single__image-item">
                                <img loading="lazy" class="h-auto" src="{{asset('uploads/products')}}/{{$productImg}}" width="674" height="674" alt="" />
                                <a data-fancybox="gallery" href="{{asset('uploads/products')}}/{{$product->image}}" data-bs-toggle="tooltip" data-bs-placement="left" title="Zoom">
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <use href="#icon_zoom" />
                                    </svg>
            ...
            <div class="product-single__addtolinks">
                @if (Cart::instance("wishlist")->content()->where("id", $product->id)->count() > 0)
                    <form action="{{route('wishlist.delete.item', ["rowId" => Cart::instance('wishlist')->content()->where("id", $product->id)->first()->rowId])}}" method="post">
                        @csrf
                        @method("DELETE")
                        <a onclick="event.target.closest('form').submit()" class="menu-link menu-link_us-s add-to-wishlist">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 24 24"><path fill="#9c7539" d="m12 21.35l-1.45-1.32C5.4 15.36 2 12.27 2 8.5C2 5.41 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.08C13.09 3.81 14.76 3 16.5 3C19.58 3 22 5.41 22 8.5c0 3.77-3.4 6.86-8.55 11.53z"/></svg>
                ...
                @else
                    <form action="{{route('wishlist.add')}}" method="post">
                        ...
                        <a onclick="event.target.closest('form').submit()" href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist">
                    ...
                    @foreach ($relatedProducts as $relatedProduct)
                        <div class="swiper-slide product-card">
                            <div class="pc__img-wrapper">
                                <a href="{{route('shop.product', ["slug"=>$relatedProduct->slug])}}">
                                    <img loading="lazy" src="{{asset('uploads/products')}}/{{$relatedProduct->image}}" width="330" height="400"  alt="Cropped Faux leather Jacket" class="pc__img">
                                    @if($relatedProduct->images)
                                        @foreach (explode(", ",$relatedProduct->images) as $relatedProductImage)
                                            <img loading="lazy" src="{{asset('uploads/products')}}/{{$relatedProductImage}}" width="330" height="400"  alt="Cropped Faux leather Jacket" class="pc__img pc__img-second">
                                        @endforeach
                                    @endif
                                </a>
                            ...
                            <div class="pc__info position-relative">
                                ...
                                <h6 class="pc__title"><a href="{{route('shop.product', ['slug'=>$relatedProduct->slug])}}">{{$relatedProduct->name}}</a></h6>
